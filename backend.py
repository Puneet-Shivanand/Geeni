        ...
        if RSIDfinder(rsid, filename):
            ...
            for row in table[1:]:
                ...
                if geneFinder(rsid, genotype, filename):
                    chromosome = "Chromosome: " + findChromosome(rsid, genotype, filename)
                    magnitude = ("Magnitude: " + cells[1].get_text().strip()) if cells[1].get_text().strip() else "No magnitude information available"
                    summary = ("Summary: " + cells[2].get_text().strip()) if cells[2].get_text().strip() else "No summary available"
                    geneInfo.append(["RSID: " + rsid, "Genotype: " + genotype, chromosome, magnitude, summary])
    return geneInfo
